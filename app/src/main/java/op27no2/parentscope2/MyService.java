                ...
                switch (message.what) {
                    case zMessageType.DATA_RECEIVED: {
                        ...
                        if(myBytes.length < 100) {
                            ...
                            //set datetime based on cal option
                            switch(calOptions) {
                                case 0:
                                    //12 hours
                                    datenear = System.currentTimeMillis();
                                    datefar = System.currentTimeMillis() - (60*60*24*500);
                                    break;
                                case 1:
                                    //24 hours
                                    datenear = System.currentTimeMillis();
                                    datefar = System.currentTimeMillis() - (60*60*24*1000);
                                    break;
                                case 2:
                                    //1 week
                                    datenear = System.currentTimeMillis();
                                    datefar = System.currentTimeMillis() - (60*60*24*1000*7);
                                    break;
                                default:
                                    // default if option 3, datefar/datenear remain as set above
                            }
                            ...
                            if(s.substring(0,11).equals("Test String")) {
                                ...
                                if(retrievalLimit.equals("No Limit")){
                                    ...
                                    System.out.println("limit is "+filesToSend);
                                }
...
/*
    ...
    public void populateList(){
        ...
        if(set ==null){
            ...
            set = myset;
            edt.putStringSet("alreadu_sent",set);
            edt.commit();
        }
        ...
        for (int i = 0; i < theNamesOfFiles.length; i++) {
                ...
                if (filelist[i].length() > 0) {
                    ...
                    if(filelist[i].lastModified()>datefar && filelist[i].lastModified() < datenear && set!=null && !set.contains(filelist[i].getName())){
    ...
    private BroadcastReceiver mMessageReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (message.equals("UPDATE PREFS")) {
                updatePrefs();
            }
    ...
    public void sendString(String mstring) {
        // populateList();

        final SharedPreferences prefs = MyApplication.getAppContext().getSharedPreferences(
                "PREFS", Context.MODE_PRIVATE);
        String deviceID = prefs.getString("bluetooth", "fail");
        System.out.println("try to send message deviceID: " + deviceID);
    ...
    private void updatePrefs(){
        System.out.println("system prefs updated");
